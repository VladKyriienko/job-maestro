...
const EditJobModal = ({ isOpen, onClose, onJobUpdated, job }: EditJobModalProps) => {
  ...
  useEffect(() => {
    if (job) {
      setFormData({
        ...
        contract_length: job.contract_length?.toString() || '',
  ...
  const handleUpdateJob = async (e: React.FormEvent) => {
    ...
    if (!job) {
      setFormError('No job selected for editing.');
      return;
    }
    setFormLoading(true);
    ...
    if (error) {
      ...
      setFormError(`Failed to update job: ${error.message}`);
      return;
    }
    onJobUpdated();
    onClose();
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose}>
      <form onSubmit={handleUpdateJob} className={styles.form}>
        <h3 className={styles.formTitle}>Edit Job</h3>
